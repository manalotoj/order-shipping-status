# tests/integration/pipelines/test_pretransit_replay.py
...
# ---- Pre-transit tracking numbers you provided ----
PRETRANSIT_TNS = [
    "393845597098",
    "393793611690",
    "393716079317",
...
def test_pretransit_rows_classified(
    tmp_path: Path, replay_dir_with_pretransit: Path, batch_size: int
):
    """
    Anchor date = 2025-10-05 (Sun). Prior week window is 2025-09-28..2025-10-04 inclusive.
    We'll set Promised Delivery Date within that window so rows survive preprocessing.
    Then assert CalculatedStatus == 'PreTransit' (rule output), not a specific carrier code.
    """
    reference_date = dt.date(2025, 10, 5)
    promised_date = "2025-10-01"  # within prior-week window
    ...
    rows = [{
        "X": "drop-me",
        "Promised Delivery Date": promised_date,
    ...
    # Run processor with replay client + fedex normalizer
...
def test_single_pretransit_smoke(tmp_path: Path, replay_dir_with_pretransit: Path):
    ...
    reference_date = dt.date(2025, 10, 5)
    promised_date = "2025-10-01"  # inside 2025-09-28..2025-10-04
